...
export function CachedResponsesService(services) {
  ...
  class CachedResponsesService {
        ...
        for (let optName in options) {
          switch (optName) {
            case 'reuseResultTime':
              ...
              break;
            case 'reuseResultOnErrorTime':
              this._reuseResultOnErrorTime = options[optName];
              break;
            case 'maxParallelRequests':
              this._maxParallelRequests = options[optName];
              break;
            case 'now':
              this._now = options[optName];
              break;
            default:
              throw new Error(`Unexpected option: ${optName}`);
          }
        }
      }
     ...
     * @private
     */
    _argsToKey(args = throwIfMissing('args')) {
  ...
  return new (require('../../common/services').Service(services)(Cyberlines))(name, configAPI.get('Ciberlines'));
  return Service(CachedResponsesService);
}
